...
names(envStack)[[1]] <- "mask" #predict onto all study region
...
###PREDICT across all models 
	for (x in 1:length(cultESlist)){
		...
		currMap <- dismo::predict(currMod, envStack, ext=extent(alpineshp), args=c('outputformat=logistic'), progress='text', filename=paste0(currOutPath,"/", cultESlist[x], "_prediction_alpine.tif"), format="GTiff", overwrite=TRUE) 
